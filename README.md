...
- Reports whether each managed hook — the scanner's `pre-commit` and the message validator's `commit-msg` — is agent-harness's own, a foreign hook, or absent.
...
Publication is the one moment the work leaves the machine. `ship` ran `qa all` and its git
guards and nothing else, so the branch convention, the commit format and the delta spec's
own verification — all already implemented, all checkable — went unenforced exactly there.
It now runs every one of them **before** the QA suite, so a refusal costs no test run:

| Guard | Refuses when | Satisfiable by |
| :--- | :--- | :--- |
| `branch check` | the branch follows no convention | renaming the branch |
| `commit check --branch` | a commit in the range does not conform | rewriting it, or `git.requireConventionalCommits: false` |
| issue key | a commit drops the key its branch names | `harness commit build`, or `git.requireIssueKey: false` |
| forbidden trailers | a commit carries one | it is empty by default; you opt in |
| `spec verify` | an **active** delta spec does not verify | finishing it, or `harness spec archive` |
...
"git": {
  ...
  "forbiddenCommitTrailers": ["Co-Authored-By: Claude", "Generated by AI"]
}
```

`forbiddenCommitTrailers` is **empty by default**. Whether AI attribution is banned is a
project policy, not a universal one — this repository's own tooling adds such a trailer
